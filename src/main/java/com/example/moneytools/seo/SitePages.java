...
public final class SitePages {
    ...
    private static final LocalDate RECENT_REFRESHED_AT = LocalDate.of(2026, 7, 27);

    private SitePages() {}

    public static final List<PageInfo> ALL = List.of(
            new PageInfo("home", "/", "홈", "머니계산기 | 실수령액·대출·세금·생활비 계산기와 가이드", "실수령액, 대출이자, 퇴직금, 배당금, 환율, 전기요금, 자동차 유지비, 생활비 계산기와 금융·세금 가이드를 한 곳에서 확인하세요.", true, true, RECENT_REFRESHED_AT),
            ...
            new PageInfo("loan", "/loan-interest-calculator", "대출이자 계산기", "대출이자 계산기 | 원리금균등·원금균등 계산", "대출금액·금리·기간을 입력하면 원리금균등·원금균등·만기일시상환의 월 납입액과 총이자를 비교합니다.", true, true, RECENT_REFRESHED_AT),
            ...
            new PageInfo("annual-salary-net", "/annual-salary-net-calculator", "연봉 실수령액 계산기", "2026 연봉 실수령액 계산기 | 세후 월급 계산", "연봉과 비과세·부양가족 조건을 입력하면 월 세후급여와 4대보험·소득세 공제액을 계산합니다.", true, true, RECENT_REFRESHED_AT),
            ...
            new PageInfo("electricity-bill", "/electricity-bill-calculator", "전기요금 계산기", "전기요금 계산기 | 가정용 누진요금 참고 계산", "월 사용량(kWh)을 입력하면 가정용 누진구간과 기본요금, 전력량요금, 부가세·기금을 반영한 예상 전기세를 계산합니다.", true, true, RECENT_REFRESHED_AT),
            new PageInfo("air-conditioner-cost", "/air-conditioner-electricity-calculator", "에어컨 전기세 계산기", "에어컨 전기세 계산기 | 하루 8시간 사용 전기요금 계산", "에어컨 소비전력과 하루 사용시간을 입력하면 월 사용량과 가정 전체 누진요금을 반영한 예상 전기세를 계산합니다.", true, true, RECENT_REFRESHED_AT),
            ...
            new PageInfo("domestic-stock-tax", "/domestic-stock-tax-calculator", "국내 주식 세금 계산기", "국내 주식 세금 계산기 | 매도세금·증권거래세 계산", "국내 주식 매도금액을 입력해 증권거래세 등 예상 매도세금을 계산하고, 국내주식과 해외주식 세금 차이를 확인해보세요.", true, true, RECENT_REFRESHED_AT),
            ...
            new PageInfo("guide-electricity-tier", "/guide/electricity-tier", "전기요금 누진세 구간 쉽게 정리", "전기요금 누진세 구간 쉽게 정리 | 가정용 전기요금 이해하기", "가정용 전기요금의 누진 구간과 기본요금, 전력량요금, 부가세를 이해하기 쉽게 정리합니다.", false, true, RECENT_REFRESHED_AT),
            new PageInfo("guide-aircon-8hours", "/guide/aircon-8hours-cost", "에어컨 하루 8시간 사용 시 전기요금 계산 방법", "에어컨 하루 8시간 사용 시 전기요금 계산 방법 | 소비전력으로 계산하기", "에어컨 소비전력과 사용 시간을 이용해 하루 8시간 기준 월 전기요금을 계산하는 방법을 설명합니다.", false, true, RECENT_REFRESHED_AT),
